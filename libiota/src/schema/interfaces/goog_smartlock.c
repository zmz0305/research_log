...
GoogSmartLock* GoogSmartLock_create(uint32_t optional_components) {
    ...
    *self = (GoogSmartLock){.base = iota_interface_create(
    ...
    if(optional_components & GoogLock_WITH_TEMPUNITSETTING) {
        self->temp_units_setting = GoogTempUnitsSetting_create("temp_unit_setting");
    }
...
void GoogSmartLock_destroy(GoogSmartLock *self) {
    ...
    if(self->lock) {
        GoogLock_destroy(self->lock);
    }
    if(self->temp_units_setting) {
        GoogTempUnitsSetting_destroy(self->temp_units_setting);
    }
...
void GoogSmartLock_get_traits(GoogSmartLock *self,
    ...
    if(self->temp_units_setting) {
        ...
        traits[trait_count] = (IotaTrait*)self->temp_units_setting;
        trait_count++;
    }
...
GoogTempUnitsSetting* GoogSmartLock_get_temp_units_setting(GoogSmartLock *self) {
    return self->temp_units_setting;
}
