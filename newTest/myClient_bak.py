...
def startDevice(device_command, api_client, email, outfile):
    try:
        ticket = api_client.registrationTickets().insert(body={"userEmail": email}).execute()
        reg_option = " --registration_ticket=" + ticket['id']
...
def reStartDevice(device_command, outfile):
    try:
        proc = subprocess.Popen(device_command, cwd="/home/wvf2/cs527-project/libweave")
